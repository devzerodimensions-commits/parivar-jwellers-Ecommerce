...
const GoldRateDisplay = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#1c1c1c] via-charcoal to-black text-cream">
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-4 py-8 sm:px-6">
        ...
        <div className="mt-8 grid w-full max-w-6xl gap-5 sm:grid-cols-3 md:mt-12 md:gap-8">
          {karats.map((r) => (
            <div
              key={r.k}
              className="rounded-2xl border border-gold-500/30 bg-gradient-to-b from-gold-500/10 to-transparent p-6 text-center backdrop-blur md:p-10"
            >
              ...
              <p className="mt-5 text-4xl font-bold md:mt-8 md:text-6xl">₹{inr(r.v)}</p>
        ...
        {data && (
          <div className="mt-8 flex items-center gap-2 text-sm text-cream/45 md:mt-12 md:text-base">
            <span className="relative flex h-2.5 w-2.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-400 opacity-75" />
              <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-green-500" />
            </span>
            {data.source === 'manual' ? 'Store rate' : 'Live rate'} · Updated {timeStr}
          </div>
        )}
      </main>
